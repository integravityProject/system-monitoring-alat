															 ...
															 <div class="form-group">
																	...
																	<div class="col-sm-6">
																		<select class="chosen-select form-control col-xs-10 col-sm-5" id="periodep" name="periode"data-placeholder="Pilih Periode" onchange="getData2()">
																			...
																			<option value="4"selected="selected">Triwulan</option>
													                				...
													                				if($tahunKe<=date('Y')){
													                						if($tahunKe < date('Y')){
																	                			if($this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == 1 ){?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}"
																                					title='Klik Untuk Pembatalan Verifikasi'><span style="color:black;" class='glyphicon glyphicon-ok'></span></a>
																                				<?php }else{?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah Anda ingin Memverifikasi?')){verifThis(<?=$val->id_kegiatan?>,<?=$b?>,4)}" title='Klik Untuk Verifikasi'><span style="color:red;" class='glyphicon glyphicon-ok'></span></a><?php
																                					}	
																                			}else if(date('m')>=$b && $tahunKe==date('Y')){
														                						if(date('m') > 3 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == 1 ){?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" 
																                					title='Klik Untuk Pembatalan Verifikasi'><span style="color:black;" class='glyphicon glyphicon-ok'></span></a>
																                				<?php }else if (date('m') < 4 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == 1) {?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" title='Klik Untuk Verifikasi'><span class='glyphicon glyphicon-ok green'></span></a>
																                				<?php }else{?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah Anda ingin Memverifikasi?')){verifThis(<?=$val->id_kegiatan?>,<?=$b?>,4)}" title='Klik Untuk Verifikasi'><span style="color:red;" class='glyphicon glyphicon-ok'></span></a><?php
																                					}
														                		...
														                		<?php
													                				...
													                				if($tahunKe<=date('Y')){
														                					if($tahunKe < date('Y')){
														                						...
														                						if(date('m') > 9 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == $b ){?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" 
																                					title='Klik Untuk Pembatalan Verifikasi'><span style="color:black;" class='glyphicon glyphicon-ok'></span></a>
																                				<?php }else if (date('m') < 10 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == 1) {?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" title='Klik Untuk Verifikasi'><span class='glyphicon glyphicon-ok green'></span></a>
																                				<?php }else{?>
																                					<a href="#"
														                		...
														                		<?php
													                				...
													                				if($tahunKe<=date('Y')){
														                					if($tahunKe < date('Y')){
														                						...
														                						if(date('m') > 12 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == $b ){?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" 
																                					title='Klik Untuk Pembatalan Verifikasi'><span style="color:black;" class='glyphicon glyphicon-ok'></span></a>
																                				<?php }else if (date('m') < 9 && $this->mpengecekan->cekSekarang($val->id_kegiatan,$tahunKe,$b,$id_alat,$id_petugas) == 1) {?>
																                					<a href="#"
																                					onclick="if(confirm('Apakah anda ingin membatalkan Verifikasi?')){cancelVerif(<?=$val->id_kegiatan?>,<?=$b?>,4)}" title='Klik Untuk Verifikasi'><span class='glyphicon glyphicon-ok green'></span></a>
																                				<?php }else{?>
																                					<a href="#"
